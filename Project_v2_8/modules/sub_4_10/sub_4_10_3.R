...
sub_4_10_3_Server <- function(id, sharedPcaData) {
  moduleServer(
    id,
...
########   Table calc. for PC2    #########################################
###########################################################################
      sharedPcaData$tbl_sign_variables_PC2 <- reactive({
        ...
        significant_df_PC2 <- data.frame(matrix(ncol = 0, nrow = nrow(merged_df2)))  # Create an empty dataframe to store significant variables
        rownames(significant_df_PC2) <- merged_df2$Dates  # Change merged_df1 to merged_df2
        
        # Perform rolling regression
        for (i in seq(1, nrow(merged_df2) - window_size, by = step_size)) {
          ...
          significant_df_PC2[i, significant_vars] <- TRUE # Store significant variables in the dataframe
        }
...
####################################################################################
      ...
      sharedPcaData$list_names_sign_PC2 <- reactive({
        significant_df_PC2 <- sharedPcaData$tbl_sign_variables_PC2()
